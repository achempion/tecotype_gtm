# Archived pain-discovery rubric

Archived on 2026-07-19.

The original long-form discovery rubric produced the sealed evidence under
`tmp/pain-discovery/` and `tmp/pain-discovery-next/`. It is not an active
acquisition gate.

Current decisions must use the concise
[acquisition gate](acquisition/readme.md#one-hard-gate). In particular:

- product usefulness, a backlink, or a marketplace listing cannot qualify a
  route;
- one channel must credibly support 10 new paid users/month;
- current evidence from the same or a tightly matched audience must support the
  full-client paid bridge;
- incremental CAC must remain below USD 50; and
- privacy and Tecotype's signed build, licensing, and updater are
  non-negotiable.

Any unknown means **Parked**, not experiment.
